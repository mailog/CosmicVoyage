...
public class Buffpewpew : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		if(gameObject.tag.Equals("EnemyPewpew"))
		{
			...
			{
				other.gameObject.GetComponent<DPS>().fireRateBuff = buff;
				other.gameObject.GetComponent<DPS>().fireRateBuffDuration = duration;
				other.gameObject.GetComponent<DPS>().buffCounter = 0;
				other.gameObject.GetComponent<DPS>().isBuffed = true;
				Destroy(gameObject);
			}
			else if(other.gameObject.name.Equals("Tank"))
			{
				other.gameObject.GetComponent<Tank>().fireRateBuff = buff;
				other.gameObject.GetComponent<Tank>().fireRateBuffDuration = duration;
				other.gameObject.GetComponent<Tank>().buffCounter = 0;
				other.gameObject.GetComponent<Tank>().isBuffed = true;
				Destroy(gameObject);
			}*/
